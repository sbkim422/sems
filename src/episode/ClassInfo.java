 ...
 * - 컴파일이나 실행 시에 참조할 클래스에 대한 부가정보, 실행 시 참조할 부가 정보를 설정하는 문법
 ...
 * - CLASS : 컴파일 후 바이트코드에 남아 있음. 
 *             실행 시에는 추출 불가! => 클래스 로딩할 때 함께 로딩되지 않음.
 * - RUNTIME : 컴파일 후 바이트코드에 남아 있음. 
 *               실행시에는 추출 가능 => 클래스 로딩할 때 애노테이션 정보도 함께 로딩됨.
 *               
 */
...
public @interface ClassInfo {
	...
	int       version() default 1; // 값을 설정하지 않으면 기본 값 1 (특이한점 디폴트가 있음)
}
