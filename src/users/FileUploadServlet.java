...
//도구사용법이라고 생각해야함 언어적으로 접근하면 실력이 멈춘다 시방쉥 정신차령
...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 3) 멀티 파트 데이터를 읽기
		// - 멀티파트로부터 각 파트를 분리하여 fileFactory에게 전달한다.
		...
		try{
			...
			for(FileItem item : items){
				if(item.isFormField()){ // 일반 데이터
					...
					System.out.print(item.getFieldName()); // 파라미터 명 출력
					System.out.print("=");
					System.out.print(item.getName()); // 파일 명 출력
					System.out.print(",");
					System.out.print(item.getContentType()); // 파일 데이터 타입
